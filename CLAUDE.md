...
`feeds` is a JSON object (`Record<number, IFeed>`) keyed by feed ID. Backend struct: `Feed` carries `URL` + its own fetch state directly. JSON uses short keys (`url`, `wm`, `bg`, `ferr`, `dd`, `dt`, `recipe`, …) — see the `Feed`/`DBCore` struct tags. `ETag`/`LastModified` are `json:"-"` (in-memory only; persisted in `seen.gz`, not db.gz).
...
- **`seen.gz`**: the persistent-dedup + fetch-cache sidecar — **a third mutable object class besides `db.gz`** (and `out/` / the self-hosted frontend shell). A single gzip blob at the store root (`backend/seen.go`): a binary dedup section (`(feed_id, guid_or_folded-title-hash) → when_seen` as unix-day, columnar so gzip RLEs the non-hash columns) plus a per-feed HTTP-validator section holding each feed's `etag`/`last_modified` (moved out of the hot db.gz). It catches **re-promotion duplicates** — a feed re-publishing an old item with a fresh `pubDate` but a stable `<guid>` that has aged out of the small `bg` snapshot, which neither `wm` (the re-dated pub sits above it) nor `bg` can suppress. Written **after** `Commit` (warn-only, write-if-dirty; `SyncSeen`) — the reverse of `hdrs`/`mp` — so a lagging sidecar only risks one duplicate next cycle / a redundant full fetch, never article loss (a pre-commit write could mark unpublished guids as seen and drop them). `wm`/`bg` deliberately **stay in db.gz** — their atomicity with the article commit is what prevents duplicate ingestion. Age-bounded (per-feed horizon `dd`, default 30d) with a per-feed flood cap (`seenFeedCap` 4096) and dead-feed purge in `evict`. Cache-Control `no-cache, must-revalidate` (like db.gz); Content-Type `application/gzip`. The frontend/service-worker **never fetch it**; a missing/corrupt `seen.gz` degrades to `bg`-only dedup + full fetches, never an error. Feed-mutation commands persist it via `db.commitState` (Commit + SyncSeen) so a `setFeedURL` reset clears the sidecar validators too. Per-feed knobs `srr feed add|upd --dedup-days N --dedup-title`; store default `srr dedup --days N`. See `backend/SEEN-POOL-PLAN.md`.
