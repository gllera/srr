...
**Single source of truth**: the Go declarations — the format constants in `backend/db.go` (`dbFormatVersion`, `idxPackSize`, `idxHeaderPrefix`, `idxEntrySize`, `idxBoundarySize`, `feedIDCeiling`, …), the write-once object-name grammar `store.PackSeries` in `backend/store/main.go` (one row per series, every stem an opaque digit run; builds the store's `packKeyRe` and, via the generated `PACK_SERIES_KINDS`, the service worker's route regex), and the JSON struct tags of `ArticleData`/`MetaEntry`/`FeedPublic`/`Manifest`/`RootState`. The TS side consumes them through the generated `frontend/src/js/format.gen.ts` (constants + grammar table + wire interfaces + the binary-layout decoders; emitted by the hidden `srr gen-ts` command, regenerated via `make generate`, freshness-checked by `make verify`). The **binary** idx layout is likewise declared once — `backend/idx_layout.go` (field, width, byte order, count source), from which `srr gen-ts` emits BOTH `backend/idx_layout.gen.go` (Go encode+decode) and format.gen.ts's decoders, so the writer (`db_pack.go`), the Go reader (`backend/idx_read.go`) and the TS reader (`frontend/src/js/idx.ts`) run one declaration instead of three hand-written mirrors. What each still owns by hand is the semantics above the bytes: the chron→data-pack bounds walk, the `ownFeedCounts` tally, the header-count shortcut and its bounds guards.
...
- Singletons carry their OWN series (`s`), so nothing hard-codes which directory a summary or a delta segment lives in. `deltas` is the ordered live chain; `seen` the dedup sidecar; `aref` the asset reference sidecar (below); `hsum`/`ssum` the idx header summary and the meta bloom summary, each with the finalized-pack count it `covers` — coverage rides NEXT to the name instead of inside it, so "the summary lags → fall back to eager idx loading" is a comparison of two numbers rather than a name-vs-count handshake.
...
`exp` (ExpireDays) is the per-feed retention window in days (0 = keep forever, max 36500) — each fetch cycle expires that feed's articles fetched longer ago: every `assets/…` object their content references is RELEASED in the asset reference sidecar and deleted once nothing references it (so a shared asset survives its first expiring referrer; the one region still deleted without a liveness check is the pre-sidecar backlog, which only drains — see `names.aref` above), and `add_idx` is bumped past them (logical deletion — packs and idx headers are immutable; entries below `add_idx` arise from expiration as well as feed id reuse). `xp` (Expired) is the cumulative expired-entry count: readers compute visible-before-pack-P as `header.feedCounts[f] − xp` once `add_idx < P.base` — per-feed `total_art` stays **all-time** because `writeIdxHeader` sources the immutable idx-header cumulative counts from it.

`cb` (ContentBytes) and `ab` (AssetBytes) are server-owned byte counters (the service worker ignores them; the reader only displays them): `cb` is the cumulative uncompressed JSONL bytes the feed's articles added to `data/` packs (bumped per article by `PutArticles`; never decreases — expiration is logical deletion, the pack bytes stay), `ab` is the live store footprint of the feed's self-hosted `assets/` objects — bumped by the stored payload size at the actual upload (content-hash dedup hits add nothing; a shared asset is charged to the feed whose fetch uploaded it first) and released, clamped at 0, when the object actually LEAVES the store, from the **owner the asset reference sidecar recorded** rather than from whichever feed happened to expire it. That is what makes it exact; two narrow residuals are stated in `db_expire.go` (an object uploaded during feed A's fetch but first stored in feed B's article is charged to A and released from B, and `srr asset heal` can change an object's size under the counter). Surfaced read-only as `content_bytes`/`asset_bytes` in `feed ls/show/edit` and `GET /api/overview`, and as the reader info cards' "Stored content" / "Stored assets" rows.
...
**Neither half (`json:"-"`)**: `etag`/`last_modified` (the incremental-fetch HTTP validators) and `bg` (BoundaryGUIDs, the FNV-32a hash array used for dedup, capped at 1024 entries — `maxBoundaryGUIDs` in `backend/feed.go` — so one misbehaving feed can't bloat the sidecar). All three live in the backend-only **seen sidecar** (CDN Layout below), hydrated onto the in-memory feed at load and written back after each fetch. ⚠ `bg` sharing that object with the seen pool does **not** make it redundant, and it must not be deleted as such (FMT3, verified 2026-07-26 — `backend/SEEN-POOL-PLAN.md` §11): the pool is age-bounded (a feed that 304s past its horizon keeps nothing, because the sweep runs every cycle over the whole pool) and gated per feed (`dd = -1` disables it), while `bg` is an un-aged, ungated snapshot of the last window — so on a dormant or opted-out **dateless** feed, where `wm` stays 0 and can never floor anything, `bg` is the only thing preventing the whole window re-ingesting.
...
Short keys: `f`=feed_id, `a`=fetched_at, `p`=published (unix seconds, omitted if 0), `t`=title (omitted if empty), `l`=link (omitted if empty), `c`=content, `g`=lang (ISO 639-1 code from the fetch pipeline, resolved in one order — a per-item value the ingest strategy declared, else fail-open detection, else the feed's own declared `<language>`/`xml:lang`; omitted if empty — absent means "unknown", including every article written before 2026-07-19). Contains all article info.
...
- **Edge cache (production deployment fact)**: the prod store's CDN host (`cdn.llera.eu`, an R2 custom domain) carries a Cloudflare Cache Rule that edge-caches the immutable prefixes with `edge_ttl: respect_origin` — the origin's `max-age=31536000, immutable` (stamped by `cacheControlForKey`) drives a year-long edge TTL, while 404s (no Cache-Control) only get Cloudflare's short default negative-cache. `db.gz`/`config.gz`/`out/` match no immutable prefix and stay uncached (plus an explicit db.gz bypass rule). ⚠ **The rule must cover `/manifest/` and `/seen/` alongside `/idx/ /data/ /meta/ /assets/`** — the manifest is the object every reader fetches on every change and the one the edge cache now buys the most on.
  **There is no purge discipline any more.** It existed because a rebuild reused finalized names with new bytes; names are never reused now, so a rebuild writes fresh stems and the edge simply never holds a stale copy of anything. `gen`, `srr gen --bump`, both purge WARNs and the service worker's purge-on-gen-change are all gone with it. (`srr asset heal` still overwrites an `assets/` key in place and still warns — a deliberate exception: `assets/` keys are content-hash-addressed and NOT listed by the manifest, so a "fresh name" for a healed object would be a *different* content-hash key than the one immutable articles reference, i.e. the repair wouldn't take effect without also rewriting every referencing article's content. That is a content-rewrite generation on top of the compaction machinery below, deliberately unbuilt — the in-place heal + WARN stays the operator repair path.)
...
- **The asset reference sidecar** (`names.aref`, `backend/asset_refs.go`, `docs/MANIFEST-SPEC.md` §4.7): the per-key live reference counts that let expiration prove an `assets/` object is dead before deleting it. Gzip JSON `{v, from, refs:{"assets/…":{n,o}}}` — `n` references, `o` the feed charged at upload — written under a FRESH stem by `SyncRefs` and published by the same Commit as the batch that changed it (fatal on failure, exactly like `SyncSeen`: a committed batch whose references went uncounted leaves every one of its keys undercounted). It rides the **`seen` series** rather than one of its own, because a manifest singleton carries its own series — so `PackSeries`, `PACK_SERIES_KINDS` and the service worker's routing are untouched, and the GC reaches it through the one rule. Backend-only; the frontend and the service worker never fetch it. `from` is the chron from which the counts are complete: at or above it the count governs and an unknown key is KEPT (a leaked object is reclaimable, wrongly-deleted shared media is not), below it the pre-refcount rule stands unchanged so an upgrading store keeps reclaiming its backlog. A sidecar the manifest names but that will not parse claims full coverage over an empty table — nothing is deleted until the counts rebuild — while a manifest naming none means no counting ever happened here.
...
- **chronIdx is a permanent address** (invariant M8): no generation may renumber chrons. Device-local state (`srr-seen` frontiers, the ★-Saved queue, shared `#pos` links, the cross-device sync blob) is keyed on it, so a rebuild or a compaction may re-encode any object under a new name but must never move an article. `srr inspect --validate` enforces M8 with a **chron-permanence** check across the last K manifests (`total_art`/`next_pid` and every same-URL feed's `total_art`/`add_idx`/`xp` only ever grow; a decrease is a renumbering and fails loudly).

- **Physical compaction (`srr compact`)** — `docs/MANIFEST-SPEC.md` §9.2, the one op in the system that removes bytes, and it is **opt-in** (default off, never run by a fetch cycle). Every article below its feed's `add_idx` is already logically deleted (expiration, or an id-reuse orphan) but its idx entry, data line and meta card slot persist because packs are immutable and chronIdx is permanent. Compaction reclaims their payload WITHOUT renumbering: each `data/` pack holding an expired line is rewritten under a **fresh stem** with that line replaced by a **tombstone** keeping `f`/`a`/`p` and dropping `t`/`l`/`c`/`g` (the absent `c` is the reader's "no longer stored" sentinel, §9.3); each `meta/` shard is rewritten with the expired card reduced to `{f,w}` (title dropped, bloom rebuilt over survivors, `ssum` republished); `idx/` is left entirely untouched (so `numFinalizedIdx`, the header cumulative counts, `hsum` and every chron→pack mapping stay valid with ZERO device-state migration); the expired articles' `assets/` objects are deleted **through the same liveness gate expiration uses** (mostly a no-op — expiration deleted them already; it never decrements, since expiration already released those references). One manifest lists the new stems at the SAME positions and flips the root; the superseded objects stay reachable through the GC grace window for stale tabs, then §7's sweep reclaims them. `total_art`, every feed's counts, `next_pid`, `pack_off`, `fetched_at` monotonicity and the strides are unchanged by construction. The live delta region is left untouched (its newest articles are essentially never expired; a rare expired delta payload compacts on a later run once it consolidates). **Reader follow-up (§9.3, out of scope here):** the reader still renders a tombstone's absent `c` as literal "undefined" — `loadArticle` parses it and title/link are already `?? ""`-guarded, but `app.ts`'s `sanitizeFragment(o.article.c)` needs an "This article is no longer stored" state. Only reachable via a ★-Saved/deep-linked expired chron (normal nav filters `chron < add_idx`), so compaction is safe to run before it lands — but it should land before compacting a public prod store.
...
- `.locked` — the store writer. Held for a whole mutation (feed/recipe/syndicate command, GUI save) — except a fetch cycle, which since FET5 (2026-07-26) holds it only for its short **write phase**: the network fan-out runs lock-free on a read-only snapshot, and the locked phase re-loads the store and folds the results back per feed (`applyFetched`, discarding any feed the store advanced underneath). Contention answers `os.ErrExist`, which serve maps to a 409 and the MCP layer to "store busy"; a cycle also fail-fast probes a LIVE foreign lease before fetching (`checkStoreBusy`, advisory) so it doesn't burn a fan-out it cannot commit.
...
**The lease** is a tiny JSON payload `{owner, expires}` written on acquire (and so re-stamped every cycle, since every cycle opens the store afresh). It exists because a nil-payload marker cannot answer the only question that matters when one is found — *is its writer still alive?* — so the answer used to be "assume yes, forever", and a SIGKILL (OOM kill, host reboot, `docker stop` past its grace) wedged the 5-minute loop until a human passed `--force`. A found marker is now reclaimed when its `owner` is **this process instance** (the in-process `storeWriter` gate proves nothing here holds it), stolen when its `expires` has passed (`leaseTTL`, 15 min — comfortably above the longest single cycle), and otherwise still refused. A payload-less marker (an older srr, a corrupt write) is refused as before: its liveness is genuinely unknowable, and `--force` is the documented escape.

**What a wrong steal costs, exactly** — the TTL is chosen against it. A stolen store cannot be double-**committed**: `publishManifest` is an exclusive create and the root flip is a CAS on the root the handle read, so the loser aborts before publishing and neither generation is written over the other. That is the guarantee, and it is a guarantee about the ROOT, **not about the objects**. Two live writers reading the same manifest draw the same per-series stems from `names.next`, and every pack write is an `AtomicPut` — overwrite, deliberately, because a crashed cycle leaves an orphan at the stem the next cycle re-draws, so exclusive-create is impossible there. The loser can therefore overwrite the winner's `data/<stem>.gz` or `idx/<stem>.gz` on its way to aborting, leaving the winner's published manifest naming objects that hold the loser's bytes. So **`leaseTTL` exceeding the longest possible cycle is a correctness requirement, not headroom**: stealing from a writer that is merely slow rather than dead is the one case the design cannot make safe (`--force` carries the same caveat and always has). If a legitimate cycle can approach the bound — a first backfill self-hosting media through an external transcode is the realistic case — raise the bound; never lower it to unwedge faster.
