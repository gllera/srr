...
| **The reader origin already differs from the store origin.** `srr.example.com` (Cloudflare Pages) reads `cdn.example.com`; there is no store-root shell (`cdn.example.com/index.html` 404s). | Cross-origin is the *normal* case, not the exotic one. CORS on each mounted origin is a live requirement (§7). |
...
`cdn.example.com` is a mounted root, production finally gets the pack cache,
...
### 7.1 CORS requirements (the `srr.example.com` lesson, generalized)
...
Operationally this is exactly what `cdn.example.com`'s R2 CORS policy already does
for `https://srr.example.com`; mounting a peer means that peer's operator does the
