...
- The reactive model: after `vi.resetModules()`, import `./model` and `./signals` with `await import(...)` in the SAME registry as the module under test — a static `import * as model` then names a different instance. Seed state through `model.<atom>.set(...)`; a hand-stamped body class is overwritten by `layout.ts`'s effect.
