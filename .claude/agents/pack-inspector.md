...
4. Run `--validate` for a full sweep. Note `[bounds-vs-data]` (checkBoundsVsData) walks every chronIdx and fetches+gunzips every distinct data pack it references (cached per pid, but a full chron walk touches the whole `data/` series; delta-region chrons resolve against the parsed chain instead); over `--url` this downloads and decompresses the entire data archive, so its cost scales with total_art and pack count (packs split at ~200 KB compressed). The other seven checks are cheap (idx-only or at most a couple of data packs).
   - `[bounds-vs-data]` — frontend lookup correctness
   - `[db-meta]` — total_art / next_pid / pack_off / per-feed total_art / per-feed add_idx
   - `[delta-chain]` — the delta-segment tail (checkDeltaChain, no-op at nd=0): invariant I2 (no 5k meta stratum inside the delta region), fetched_at monotone within the chain, and fetched_at monotone across the pack↔delta seam (last consolidated article at chron tc−1 vs the first delta)
   ...
   - `[latest-files]` — the consolidated tail packs `idx/L<tailGen>.gz` and `data/L<tailGen>.gz` present (`tailGen = seq − nd`, via latestKey); an all-delta store (`tailCovered == 0`) correctly expects none
...
- `chron N: packId=K offset=X >= entries=Y (frontend crashes on this chronIdx)` — the idx pack's bounds resolve `chron N` to a data-pack offset that doesn't exist. In the live frontend this out-of-range offset is caught in `data.ts loadArticle` (it drops the cache and throws a `pack … out of sync … retry to refresh` Error on an out-of-range offset; delta-region chrons have their own variant, `delta chain out of sync at chron N`); historically, before that guard, it surfaced as the `(reading 'f')` TypeError when the undefined article reached `showFeed`'s `article.f` access in `nav.ts`. Investigate writer/reader symmetry (see `idx-format-reviewer` agent) or stale pack files (tail generation `L<tailGen>.gz`, delta segments `d<g>.gz`, finalized `N.gz` — e.g. an in-place store rebuild reusing names without a `gen` bump).
