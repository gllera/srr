...
- **`telegram.go`** — Built-in `#telegram` strategy (registered inline in `init`): scrapes `https://t.me/s/<channel>` HTML via `golang.org/x/net/html`. One `RawItem` per `div.tgme_widget_message` — `data-post` (e.g. `channel/123`) → hashed into GUID, permalink as Link; `time[datetime]` inside `a.tgme_widget_message_date` → Published. Content composes media + text in DOM order: `a.tgme_widget_message_photo_wrap` → `<p><a href=url><img src=url></a></p>` (URL from nested `<img>` or inline `background-image:url(…)`); `a.tgme_widget_message_video_player` / `tgme_widget_message_roundvideo` → when `<video src>` is present, an inline `<video>` player (`controls preload="metadata" playsinline`) with the thumbnail as `poster` and `width`/`height` derived from the wrapper's `padding-top:NN%` aspect-ratio hint (so the element starts at hint-derived dimensions instead of the 320×180 poster size; any residual reflow on first play comes from the hint disagreeing with the real mp4 ratio); otherwise falls back to a clickable thumbnail linking to the message permalink; `div.tgme_widget_message_text` → raw inner HTML (raw — the sanitize/minify pipeline modules are the right place to clamp it). No ETag/Last-Modified: relies on per-feed GUID dedup to suppress re-presented messages.
