...
func TestClearCacheCmdRemovesCacheDir(t *testing.T) {
	cacheDir := t.TempDir()
	globals = &Globals{Cache: cacheDir}

	if err := os.WriteFile(filepath.Join(cacheDir, "sentinel"), []byte("x"), 0644); err != nil {
		t.Fatalf("write sentinel: %v", err)
	}

	if err := (&ClearCacheCmd{}).Run(); err != nil {
		t.Fatalf("ClearCacheCmd.Run: %v", err)
	}

	if _, err := os.Stat(cacheDir); !os.IsNotExist(err) {
		t.Errorf("cache dir %s should have been removed", cacheDir)
	}
}

func TestClearCacheCmdWorksWithoutCacheDir(t *testing.T) {
	globals = &Globals{}

	if err := (&ClearCacheCmd{}).Run(); err != nil {
		t.Fatalf("ClearCacheCmd.Run with no cache: %v", err)
	}
}

// Regression: NewCache failure used to overwrite `backend` with a nil *Cache,
// so the subsequent backend.Close() dereferenced a nil receiver and panicked.
func TestNewDBHandlesCacheInitFailure(t *testing.T) {
	storeDir := t.TempDir()
	...
	globals = &Globals{Store: storeDir, Cache: blocker}

	_, err := NewDB(ctx, false)
	if err == nil {
		t.Fatal("NewDB should fail when cache init fails")
	}
	if !strings.Contains(err.Error(), "initialize cache") {
		t.Errorf("err = %v, want 'initialize cache' wrap", err)
	}
}
