...
func fetchOnce(t *testing.T, feed *Feed, ch *Channel, srv *httptest.Server) []*Item {
	...
	items, err := feed.fetch(context.Background(), srv.Client(), buf, mod.New(nil), ingest.New(ingest.Deps{}), ch, fetchedAt, ch.Pipe, ingest.Select(ch.Ingest, ""))
	if err != nil {
...
func TestFetchFutureDatedItemClampedToFetchedAt(t *testing.T) {
	...
	buf := make([]byte, 1<<20)
	items, err := f.fetch(context.Background(), srv.Client(), buf, mod.New(nil), ingest.New(ingest.Deps{}), ch, fetchedAt, ch.Pipe, ingest.Select(ch.Ingest, ""))
	if err != nil {
		t.Fatalf("fetch: %v", err)
	}
	if len(items) != 1 {
