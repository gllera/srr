...
// chan_id widened from u8 to u16: this exercises ids ≥ 256 end-to-end —
// register 300 channels, write one article each through PutArticles, then
// round-trip the latest idx pack through parseIdxPack and run the full
// inspect --validate sweep. Pre-widen, the entry's single chan_id byte would
// alias id 256 → 0, 257 → 1, … and the variable header (numSlots) would not
// reach 300.
func TestChanWidthBeyond255(t *testing.T) {
	db, c, _ := setupTestDB(t)

	const n = 300
	// One article per channel; ids 0..n-1. Each channel needs a non-empty URL
	// so the inspect --validate reopen (NewDB) passes the feed→channel migration
	// guard.
	c.Channels = make(map[int]*Channel, n)
	articles := make([]*Item, 0, n)
	for id := range n {
		ch := &Channel{id: id, Title: fmt.Sprintf("ch%d", id), URL: fmt.Sprintf("https://example.com/%d", id)}
		c.Channels[id] = ch
	...
	// Round-trip the latest idx pack: every entry's chan_id must survive,
	...
	for _, id := range pack.chanIDs {
		seen[id] = true
	}
	for id := range n {
		if !seen[uint16(id)] {
			t.Errorf("chan_id %d missing from parsed idx entries", id)
		}
		if got := pack.ownChanCount(id); got != 1 {
			t.Errorf("ownChanCount(%d) = %d, want 1", id, got)
		}
	}
	// A high id must not alias a low one: id 256 distinct from id 0.
	if seen[256] != true {
		t.Error("chan_id 256 not parsed (u8 aliasing would lose it)")
	}
