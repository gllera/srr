...
type Globals struct {
	...
	AssetProcess string `                             env:"SRR_ASSET_PROCESS" help:"Command run on every self-hosted asset just before upload to process its bytes, e.g. transcode media (the cache file path is appended as the final arg; processed bytes are read from stdout; non-zero exit or empty output keeps the original). Skipped when the source is already uploaded. Empty disables. E.g. \"webify -m 720\"."`
