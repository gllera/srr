...
type Globals struct {
	...
	AssetProcessTimeout time.Duration `        env:"SRR_ASSET_PROCESS_TIMEOUT" default:"60m" help:"Timeout for a single asset-process command invocation (Go duration). Separate from --cmd-timeout because media transcoding can run far longer than a feed/ingest command. asset-peek stays on --cmd-timeout."`
