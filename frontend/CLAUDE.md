...
| `idx.ts` | Binary idx pack parsing. Imports the format constants from `format.gen.ts`; re-exports `IDX_PACK_SIZE`. Exports `IdxPack` interface, `makeIdxPack()`. |
...
**Init**: `data.init()` loads all idx packs in parallel, calls `makeIdxPack()` (from `idx.ts`) to lazily parse each binary pack into `chanIds`/`fetchedAts` typed arrays and `bounds`. Latest packs are generation-named `L<seq>.gz` (`db.seq`, normalized `??= 0` for the omitempty empty-store case); every pack name is write-once, so all pack fetches use `cache: "force-cache"`. A non-OK response on a latest pack means this tab's db.gz predates the backend GC grace window — `assertPackOk` triggers one `location.reload()` (sessionStorage-guarded against loops, guard cleared after a successful `init()`) and always throws so callers never touch the body.
