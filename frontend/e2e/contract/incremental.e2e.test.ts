...
// only appends the new items, the latest-pack toggle (data_tog) flips, and the
// reader still round-trips all articles in published order.

describe("contract: incremental fetch + dedup + toggle", () => {
   ...
   it("second fetch appended new-only (dedup held) and flipped data_tog", () => {
      expect(reader.data.db.total_art).toBe(5)
      expect(reader.data.db.data_tog).toBe(!tog1)
   })
   ...
   it("backend inspect --validate agrees after the toggle", async () => {
